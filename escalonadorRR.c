...
void * _RR(void *node){
    ...
    while(*filaPronto != NULL && getTime()-auxTimer < tq){
        ...
        printf("Timer informa ao Escalonador Round-Robin de CPU que o processo %d atualmente em execução precisa ser retirado da CPU\n", newProcess->id);
        printf("Escalonador Round-Robin de CPU escolheu o processo %d, retirou-o da fila de prontos e o encaminhou ao Despachante\n", newProcess->id);
